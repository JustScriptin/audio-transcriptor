 ...
 * @returns {Promise<string[]>} An array of paths corresponding to the chunked files or the original file if its size is under 20 MB.
 * @example
 * ```
...
const chunkMp3File = async (mp3FilePath: string): Promise<string[]> => {
  ...
  try {
    ...
    const { duration: fileDuration = 0, format_name: inputFormat = '' } = fileData.format ?? {};
    ...
    if (fileSizeInMB <= 20) {
      ...
      const targetChunkDurationInSeconds = (targetChunkSizeInBytes / stats.size) * fileDuration;
      ...
      await new Promise<void>((resolve, reject) => {
        ffmpeg(mp3FilePath)
          ...
          .format('mp3')
          .outputOptions('-map', '0', '-segment_time', targetChunkDurationInSeconds.toString(), '-f', 'segment')
      ...
      // Saves file paths of chunked files that are greater than or equal to 180 KB but smaller than 20 MB into an array to be returned
