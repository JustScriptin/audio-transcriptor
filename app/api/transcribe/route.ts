 ...
 * @returns {Promise<Response>} A promise that resolves to a JSON response containing the transcription result.
 * @example
 * ```
...
export const POST = async (req: Request): Promise<Response> => {
  ...
  const inputFilePath = path.join(process.cwd(), "public", "uploads", file.name);
  ...
  const promises = chunkedMp3Paths.map(async (chunkedMp3Path): Promise<{ output?: string; error?: any }> => {
  ...
  const { output: transcription } = result.reduce(
    (acc, cur) => ({ output: acc.output ? acc.output + cur.output : cur.output }),
    {}
  );
  ...
  return new Response(JSON.stringify({ fileName: `${fileName}.txt`, transcription }), { status: 200 });
};